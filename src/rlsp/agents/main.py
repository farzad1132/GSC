...
def set_random_seed(seed, agent_helper):
    """Set random seed for all involved libraries (random, numpy, torch)"""
...
def create_agent(agent_helper):
    ...
    if agent_type == "DDPG":
        ...
        raise Exception("DDPG is not yet supported in pytorch version")
    elif agent_type == "SAC":
        agent = SAC_Agent(agent_helper, "test")
    return agent
...
if __name__ == '__main__':
    agent_config = 'res/config/agent/sac_agent.yaml'
